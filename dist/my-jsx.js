    ...
    if (node.children) {
        node.children.forEach((child, index) => {
            if (typeof child !== "string") {
                console.log("inserting", child?.id || child?.children?.[0], "to", node.id, index);
...
export function render(ctx, node, parent) {
    if (node.id === "root") {
    ...
    if (node.tag === "text") {
        ...
        ctx.fillText(node.children[0] || "", left + (parent?.yogaNode.getComputedLeft() || 0), top + height + (parent?.yogaNode.getComputedTop() || 0), width);
    }
    else if (node.tag === "box") {
        console.log("drawing box", node.id, layout);
        ctx.fillStyle = node.style.backgroundColor;
        ctx.fillRect(left + (parent?.yogaNode.getComputedLeft() || 0), top + (parent?.yogaNode.getComputedTop() || 0), width, height);
        if (node.children) {
            node.children.forEach((child, index) => {
